...
import { likePlace } from '/src/apis/placeAPI';
...
function PlaceItem({
  ...
  // 장소 좋아요
  const toggleLiked = async (placeId, event) => {
    const res = await likePlace(placeId);
    if (res) {
      setPlaces((prevPlaces) =>
        prevPlaces.map((place) => (place.placeId === placeId ? { ...place, liked: !place.liked } : place)),
      );
    }
  };
  // 별점 관리 상태 추가
          ...
          <ItemDetails>
            <ItemTitle>
              ...
              {!isModal && (
                <ItemInfo
                  src={informationIcon}
                  onClick={() => showModal({ name, srcImg, rate, category, floor, startTime, endTime, phone })}
                ></ItemInfo>
              )}
            </ItemTitle>
            <ItemRating>
            ...
            <FooterDetails>
              {floor}층 | {formatTime(startTime)} - {formatTime(endTime)}
            </FooterDetails>
          </ItemDetails>
          {currentUrl !== '/coursecreate' && !isModal && (
            <HeartIcon src={liked ? heartFilled : heartEmpty} onClick={toggleLiked} />
          )}
          {currentUrl === '/coursecreate' && (liked ? <HeartIcon src={heartFilled} /> : null)}
      ...
      <Modal title="장소 정보" open={open} onCancel={handleCancel} footer={null} width={380}>
        {modalText ? (
          <>
            ...
            <MyRatingContainer>
              <strong>나의 평점</strong>
              <CustomRate value={userRate} allowHalf onChange={(value) => setUserRate(value)} />
            </MyRatingContainer>
            <div>
              <strong>전화번호</strong> : {modalText.phone} <br />
              <br />
              <strong>영업시간</strong>
              <br />월 : {formatTime(modalText.startTime)} ~ {formatTime(modalText.endTime)}
              <br />화 : {formatTime(modalText.startTime)} ~ {formatTime(modalText.endTime)}
              <br />수 : {formatTime(modalText.startTime)} ~ {formatTime(modalText.endTime)}
              <br />목 : {formatTime(modalText.startTime)} ~ {formatTime(modalText.endTime)}
              <br />금 : {formatTime(modalText.startTime)} ~ {formatTime(modalText.endTime)}
              <br />토 : {formatTime(modalText.startTime)} ~ {formatTime(modalText.endTime)}
              <br />일 : {formatTime(modalText.startTime)} ~ {formatTime(modalText.endTime)}
              <br />
            </div>
          </>
        ) : (
