...
import { PostButton, PlaceButton, Tabs, Tab, FilterContainer, LikeContainer, LikeTab } from './LikePlace.style';
...
function LikePlace() {
  ...
  const handlePlaceLikeToggle = (place_id) => {
    setPlaceDummy((prevPlaces) =>
      prevPlaces.map((place) => (place.place_id === place_id ? { ...place, liked: !place.liked } : place)),
    );
  };
  ...
  return (
    <>
      <Tabs>
        <Tab active={activeTab === 'myCourses'} onClick={() => handleTabClick('myCourses')}>
          나의 코스
        </Tab>
        <LikeTab active={activeLikeTab} onClick={() => handleTabClick('likeCourses')}>
          좋아요
        </LikeTab>
      </Tabs>
      ...
      {activeTab !== 'myCourses' && (
        <FilterContainer>
          <PlaceButton active={activeTab === 'likedPlaces'} onClick={() => handleTabClick('likedPlaces')}>
            장소
          </PlaceButton>
          <PostButton active={activeTab === 'likeCourses'} onClick={() => handleTabClick('likeCourses')}>
            코스
          </PostButton>
        </FilterContainer>
      )}
      ...
      <div>
        {activeTab === 'likedPlaces' && (
          <LikeContainer>
            {likePlaces.map(
              ({ place_id, name, phone, start_time, end_time, floor, location, category, rate, liked }) => (
